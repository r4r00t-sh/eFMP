...
export class MinIOService implements OnModuleInit {
  ...
  constructor(private configService: ConfigService) {
    this.internalEndpoint = this.configService.get<string>(
      'MINIO_ENDPOINT',
      'localhost',
    );
    this.internalPort = parseInt(
    ...
    this.publicPort = parseInt(
      this.configService.get<string>('MINIO_PUBLIC_PORT', '9000'),
      10,
    );
    const useSSL =
  ...
  async getFileUrl(
    ...
    if (
      ...
      this.internalPort === this.publicPort
    ) {
      return url;
    }
